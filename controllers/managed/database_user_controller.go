...
// +kubebuilder:rbac:groups=managed.upcloud.kubeform.com,resources=databaseusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=managed.upcloud.kubeform.com,resources=databaseusers/status,verbs=get;update;patch

func (r *DatabaseUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("databaseuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatabaseUser")
...
func (r *DatabaseUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managedv1alpha1.DatabaseUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", managedv1alpha1.DatabaseUser{}.APIVersion, managedv1alpha1.DatabaseUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managedv1alpha1.DatabaseUser{}).
...
func (r *DatabaseUserReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for DatabaseUser")
			return nil
		}
		...
		resourceList := &managedv1alpha1.DatabaseUserList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DatabaseUser")
			return nil
		}
