...
// DatabaseMysqlReconciler reconciles a DatabaseMysql object
type DatabaseMysqlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=managed.upcloud.kubeform.com,resources=databasemysqls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=managed.upcloud.kubeform.com,resources=databasemysqls/status,verbs=get;update;patch

func (r *DatabaseMysqlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("databasemysql", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatabaseMysql")
...
func (r *DatabaseMysqlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managedv1alpha1.DatabaseMysql{}); err != nil {
			klog.Error(err, "unable to set up auditor", managedv1alpha1.DatabaseMysql{}.APIVersion, managedv1alpha1.DatabaseMysql{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managedv1alpha1.DatabaseMysql{}).
...
func (r *DatabaseMysqlReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for DatabaseMysql")
			return nil
		}
		...
		resourceList := &managedv1alpha1.DatabaseMysqlList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DatabaseMysql")
			return nil
		}
