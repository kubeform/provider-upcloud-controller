...
// DatabasePostgresqlReconciler reconciles a DatabasePostgresql object
type DatabasePostgresqlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=managed.upcloud.kubeform.com,resources=databasepostgresqls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=managed.upcloud.kubeform.com,resources=databasepostgresqls/status,verbs=get;update;patch

func (r *DatabasePostgresqlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("databasepostgresql", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatabasePostgresql")
...
func (r *DatabasePostgresqlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managedv1alpha1.DatabasePostgresql{}); err != nil {
			klog.Error(err, "unable to set up auditor", managedv1alpha1.DatabasePostgresql{}.APIVersion, managedv1alpha1.DatabasePostgresql{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managedv1alpha1.DatabasePostgresql{}).
...
func (r *DatabasePostgresqlReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for DatabasePostgresql")
			return nil
		}
		...
		resourceList := &managedv1alpha1.DatabasePostgresqlList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DatabasePostgresql")
			return nil
		}
