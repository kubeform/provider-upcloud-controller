...
// +kubebuilder:rbac:groups=floating.upcloud.kubeform.com,resources=ipaddresses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=floating.upcloud.kubeform.com,resources=ipaddresses/status,verbs=get;update;patch

func (r *IpAddressReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipaddress", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpAddress")
...
func (r *IpAddressReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &floatingv1alpha1.IpAddress{}); err != nil {
			klog.Error(err, "unable to set up auditor", floatingv1alpha1.IpAddress{}.APIVersion, floatingv1alpha1.IpAddress{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&floatingv1alpha1.IpAddress{}).
