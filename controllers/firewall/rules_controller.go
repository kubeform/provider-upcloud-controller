...
// +kubebuilder:rbac:groups=firewall.upcloud.kubeform.com,resources=rules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=firewall.upcloud.kubeform.com,resources=rules/status,verbs=get;update;patch

func (r *RulesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("rules", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Rules")
...
func (r *RulesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &firewallv1alpha1.Rules{}); err != nil {
			klog.Error(err, "unable to set up auditor", firewallv1alpha1.Rules{}.APIVersion, firewallv1alpha1.Rules{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&firewallv1alpha1.Rules{}).
